...
Define the neural network for S-GMGM, including the following classes:
...
from pkgs.models.sgmgm import layers_sgmgm as model
...
class GroundMotionDatasets(Dataset):
    ...
    def __init__(self, csv_path: Path, label_list: List[str]) -> None:
        ...
        ----------
        :param csv_path: str
            The path to the csv file containing the path of ground-motion data and the values of the corresponding label data.
    ...
    def __getitem__(self, index):
        ...
        temp_mat = np.load(path, allow_pickle=False)
...
class MappingNetwork(nn.Module):
    ...
    def __init__(self, z_dim: int, w_dim: int, start_size: int = 16, end_size: int | None = None) -> None:
        """
        Parameters
        ----------
        z_dim : int
