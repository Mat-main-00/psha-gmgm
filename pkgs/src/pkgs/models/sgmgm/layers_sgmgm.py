...
S-GMGM (StyleGAN-based Ground-Motion Generative Model)
Define the core classes and functions required to construct S-GMGM neural networks.
...
Most parts of this script are identical to those in my other repository:
...
def noise_normalization(input_noise: torch.Tensor) -> torch.Tensor:
    ...
    Applied to the input of the Mapping network.
    """
...
class EqualizedLinear(nn.Module):
    ...
    def __init__(self, in_dim: int, out_dim: int, lr_mul: float):
        """
        Parameters
        ----------
        in_dim : int
...
class AddBiasChannelWise(nn.Module):
    ...
    def __init__(self, out_c: int, bias_scale: float):
...
class BlurPooling(nn.Module):
    ...
    def __init__(self, in_c: int):
        ...
        blur_kernel = np.array([1.0, 3.0, 3.0, 1.0], dtype=np.float32)
        blur_filter = torch.from_numpy(blur_kernel)
        blur_filter = blur_filter / torch.sum(blur_filter)
        blur_filter = blur_filter.view(1, 1, 1, 4).expand(in_c, 1, 1, 4)
        blur_filter = blur_filter.contiguous()
...
class UpSampleWave(nn.Module):
    ...
    def __init__(self, in_c: int = 1):
        ...
        :param in_c: The number of channels of generated ground motion data.
                     ...
                     If it id three-component, in_c=3.
        """
...
class PixelWiseNoise(nn.Module):
    ...
    def __init__(self, wave_width: int):
        """
        Parameters
        ----------
...
class Conv2dLayer(nn.Module):
    ...
    def __init__(self, in_c, out_c, kh, kw, pad, stride, lr_mul):
        """
        Parameters
        ----------
        :param in_c:      The number of input channels.
...
class FromWave(nn.Module):
    ...
    def __init__(self, in_c: int, out_c: int, lr_mul: float):
        """
        Parameters
        ----------
        :param in_c:   The number of input channels.
...
class DownSampleConv2d(nn.Module):
    def __init__(self, in_c: int, out_c: int, kw: int):
        """
        Parameters
        ----------
        :param in_c:   The number of input channels.
...
class BlurPoolDiscriminator(nn.Module):
    ...
    def __init__(self, in_c: int, pad: int):
        """
        Parameters
        :param in_c:  The number of input channels.
