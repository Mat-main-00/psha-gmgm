...
CS-GMGM (Conditional StyleGAN-based Ground-Motion Generative Model)
Define the core classes and functions required to construct CS-GMGM neural networks.
...
This code was created with reference to the following repositories:
    ...
    ・https://github.com/konstantinjdobler/multi-conditional-stylegan.git
    ・https://github.com/cedricoeldorf/ConditionalStyleGAN.git

Substantial portions of this script are identical to those in my other repository:
    https://github.com/Mat-main-00/ss_gmgm.git
"""
...
def noise_normalization(input_noise: torch.Tensor) -> torch.Tensor:
    """
...
class EqualizedLinear(nn.Module):
    """
    Fully connected layer with equalized learning rate and custom learning rate multiplier.
    """
...
class AddBiasChannelWise(nn.Module):
    """
    Add channel-wise bias to the input tensor.
    """
...
class ModulateConv2d(nn.Module):
    ...
    def __init__(
        ...
        # Layer for affine transformation of style, w
...
class BlurPooling(nn.Module):
    """
    Blur pooling layer.
    Designed for 1D time-series data (ground motions) and may not be suitable for other data shapes.
    """

    const_blur_filter: torch.Tensor
    ...
    def __init__(self, in_c: int) -> None:
        ...
        blur_filter = blur_filter.expand(in_c, 1, 1, 4).contiguous().clone()
...
class PixelWiseNoise(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        noise = self.const_noise.expand(b_size, 1, hei, wid).contiguous().clone()
...
class Conv2dLayer(nn.Module):
    ...
    def __init__(self, in_c, out_c, kh, kw, pad, stride, lr_mul) -> None:
        """
        Parameters
        ----------
        :param in_c:      The number of input channels.
...
class BlurPoolDiscriminator(nn.Module):
    """
    Blur pooling layer for discriminator.
    """
...
def discriminator_loss_r1(
    disc_real_out: torch.Tensor,
    reals_f: torch.Tensor,
    gamma_f: float = 10,
    d_reg_int: int = 16,
) -> torch.Tensor:
...
class GeneratorLossPathRegularization(nn.Module):
    def __init__(self, device, pl_decay: float = 0.01, pl_weight: float = 2.0, g_reg_int: int = 4):
