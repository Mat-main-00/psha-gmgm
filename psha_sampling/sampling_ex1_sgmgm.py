...
for ll in range(finish_num):
    # Sample the number of earthquake occurrence
    ...
    mw = ffm.sample_m(uni, b, mu, ml)
    temp_r = rng.uniform(0, 1, num_event)
    r = area_r * np.sqrt(temp_r)
    r = np.sqrt(r**2 + depth**2)

    for ii in range(num_event):
        catalog.append([ll + 1, ii, mw[ii], r[ii], t_vs30])
...
split_indices = np.array_split(indices, n_split)

vs30_low = t_vs30 - vs30_diff
vs30_high = t_vs30 + vs30_diff

for part_id, part_idx in enumerate(split_indices, start=1):
    part_df = catalog_df.copy().iloc[part_idx].reset_index(drop=True)
    n_part = len(part_df)
    print(f"Processing part {part_id}/{n_split}: {n_part} events")

    # target Mw, R, Vs30
    mw_target = part_df["mw"].to_numpy()
    r_target = part_df["r"].to_numpy()
    vs30_target = part_df["vs30"].to_numpy()
    ...
    while not np.all(assigned):
        ...
        print("Rem: ", len(remaining), f"Part: {part_id}")

    # ===================== このブロックの保存 =====================
    if not np.all(assigned):
        print(f"Warning: part {part_id} not fully assigned. Remaining: {np.sum(~assigned)}")
    else:
