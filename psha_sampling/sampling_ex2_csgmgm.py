...
PSHA sampling based on the CS-GMGM for numerical example 2.
"""
...
while flag:
    ...
    for ff in range(len(mw_list)):
        ...
        if num_event == 0:
            ...
            noise = torch.randn(size=(num_event, z_dim), device=device)
            ...
            save_int_mat.append(save_int.reshape(-1, w_dim))
...
save_int_tmp = np.concatenate(save_int_mat, axis=0)
...
np.save(save_dir / f"sample_int_ex2_epoch_{epoch}.npy", save_int_tmp)
