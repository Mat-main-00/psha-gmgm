...
PSHA sampling based on the CS-GMGM for numerical example 1.
Executed with a CPU.
"""
...
from pkgs.models.csgmgm import networks_csgmgm
...
model_file = f"../data/model_CS_G_epoch_{epoch}.pth"

# Dataset label
label_file = f"../data/dataset_csgmgm.csv"
...
netG = networks_csgmgm.Generator(z_dim=z_dim, c_dim=label_dim - 1, w_dim=w_dim, wave_len=8192).to(device)
...
for jj in range(div_num):
    ...
    for ll in range(loop_num):
        ...
        noise = torch.randn(size=(len(mw_list_norm), z_dim), device=device)

        # Generate ground-motions
        with torch.no_grad():
            fake_wave, fake_int, fake_pga = netG(noise, label_mat_t, is_train=False)
        save_acc = fake_wave.squeeze().to("cpu").detach().numpy()
        save_int = fake_int.squeeze().to("cpu").detach().numpy()
    ...
    save_inter = np.concatenate(save_int_mat, axis=0)
    save_label = np.concatenate(save_label_mat, axis=0)

    np.save(save_dir / f"sample_wave_poisson{jj + 1}_all_epoch_{epoch}_cpu.npy", save_mat)
    np.save(save_dir / f"sample_int_poisson{jj + 1}_all_epoch_{epoch}_cpu.npy", save_inter)
    save_df = pd.DataFrame(save_label, columns=pd.Index(["mw", "fault_dist", "v30", "gen_pga", "sim_num"]))
    save_df.to_csv(save_dir / f"sample_label_poisson{jj + 1}_all_epoch_{epoch}_cpu.csv", index=False)
...
# Save in log file
with open("time_log_cs_cpu.txt", "a") as f:
